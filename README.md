...
Pin a version: `VERSION=v0.1.0 sh install.sh`. The script installs to `/usr/local/bin` (asks for sudo if needed). Windows users download the `.exe` from the releases page and put it on PATH.
